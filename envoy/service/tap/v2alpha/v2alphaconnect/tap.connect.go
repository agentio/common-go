...
import (
	...
	context "context"
	errors "errors"
	v2alpha "github.com/agentio/common-go/envoy/service/tap/v2alpha"
	http "net/http"
	strings "strings"
)
...
const (
	...
	// RPC.
	TapSinkServiceStreamTapsProcedure = "/envoy.service.tap.v2alpha.TapSinkService/StreamTaps"
)

// These variables are the protoreflect.Descriptor objects for the RPCs defined in this package.
var (
	tapSinkServiceServiceDescriptor          = v2alpha.File_envoy_service_tap_v2alpha_tap_proto.Services().ByName("TapSinkService")
...
type TapSinkServiceClient interface {
	...
	StreamTaps(context.Context) *connect.ClientStreamForClient[v2alpha.StreamTapsRequest, v2alpha.StreamTapsResponse]
}

// NewTapSinkServiceClient constructs a client for the envoy.service.tap.v2alpha.TapSinkService
...
func NewTapSinkServiceClient(httpClient connect.HTTPClient, baseURL string, opts ...connect.ClientOption) TapSinkServiceClient {
	...
	return &tapSinkServiceClient{
		streamTaps: connect.NewClient[v2alpha.StreamTapsRequest, v2alpha.StreamTapsResponse](
			httpClient,
...
type tapSinkServiceClient struct {
	streamTaps *connect.Client[v2alpha.StreamTapsRequest, v2alpha.StreamTapsResponse]
}

// StreamTaps calls envoy.service.tap.v2alpha.TapSinkService.StreamTaps.
func (c *tapSinkServiceClient) StreamTaps(ctx context.Context) *connect.ClientStreamForClient[v2alpha.StreamTapsRequest, v2alpha.StreamTapsResponse] {
	return c.streamTaps.CallClientStream(ctx)
}

// TapSinkServiceHandler is an implementation of the envoy.service.tap.v2alpha.TapSinkService
// service.
type TapSinkServiceHandler interface {
	// Envoy will connect and send StreamTapsRequest messages forever. It does not expect any
	// response to be sent as nothing would be done in the case of failure. The server should
	// disconnect if it expects Envoy to reconnect.
	StreamTaps(context.Context, *connect.ClientStream[v2alpha.StreamTapsRequest]) (*connect.Response[v2alpha.StreamTapsResponse], error)
}
...
func NewTapSinkServiceHandler(svc TapSinkServiceHandler, opts ...connect.HandlerOption) (string, http.Handler) {
	...
	return "/envoy.service.tap.v2alpha.TapSinkService/", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
...
func (UnimplementedTapSinkServiceHandler) StreamTaps(context.Context, *connect.ClientStream[v2alpha.StreamTapsRequest]) (*connect.Response[v2alpha.StreamTapsResponse], error) {
	return nil, connect.NewError(connect.CodeUnimplemented, errors.New("envoy.service.tap.v2alpha.TapSinkService.StreamTaps is not implemented"))
}
